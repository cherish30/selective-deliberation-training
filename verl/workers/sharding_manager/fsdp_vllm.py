    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        if peft_config:
            if self.base_sync_done:
                ...
                # Use base model path as lora_path to avoid tokenizer loading warnings
                # Since we pass weights directly via lora_tensors, the actual path content doesn't matter
                # but vLLM requires a valid path that can be used for tokenizer loading
                base_model_path = self.model_runner.model_config.model
                lora_reqest = TensorLoRARequest(
                    lora_name=f"{lora_int_id}",
                    lora_int_id=lora_int_id,
                    lora_path=base_model_path,

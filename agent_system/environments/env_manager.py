...
# Modified by Anonymous Authors, 2026: added AlfWorldSelectiveThinkingEnvironmentManager and gate-step index fix.
...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        # Gate steps (is_gate_step=True) are in total_batch_list but have no
        # corresponding entry in total_infos (only env-stepping rows do).
        # Build a mapping: batch_list_index -> total_infos_index.
        info_idx = 0
        batch_to_info: dict = {}
        for idx, item in enumerate(total_batch_list[batch_idx]):
            if not item.get('is_gate_step', False):
                batch_to_info[idx] = info_idx
                info_idx += 1

        # Find the last active non-gate entry.
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            batch_item = total_batch_list[batch_idx][i]
            if batch_item['active_masks'] and i in batch_to_info:
                info = total_infos[batch_idx][batch_to_info[i]]
                won_value = float(info['won'])
                success['success_rate'].append(won_value)
...
class AlfWorldSelectiveThinkingEnvironmentManager(AlfWorldEnvironmentManager):
    """ALFWorld manager that drives the selective-thinking rollout protocols.

    Two modes are supported:
      - dt_action       : the outer model emits `Action: deep_think` to call
                          the internal deep-thinking tool. The next outer
                          step disables the deep_think token (one-step
                          cooldown) so the agent must execute a real env
                          action.
      - selective_gate  : a separate gate generation decides whether to call
                          the deep_think tool. On true the rollout loop
                          performs an extra DT generation, then the action
                          generation uses the resulting memo as guidance.

    The actual DT/gate model calls live in the rollout loop because they
    depend on the actor's `generate_sequences` worker group. This manager
    only owns: prompt construction, memo bookkeeping, and DT-call counters.
    """
    ...
    def __init__(self, envs, projection_f, config):
        ...
        mode = getattr(config.env, "thinking_mode", "dt_action")
        if mode not in ("dt_action", "selective_gate"):
            raise ValueError(
                f"AlfWorldSelectiveThinkingEnvironmentManager requires "
                f"thinking_mode in (dt_action, selective_gate); got {mode!r}"
            )
        self.thinking_mode = mode
        self._TrajectoryDTState = TrajectoryDTState
        self._dt_states: List = []

    def reset(self, kwargs):
        observations, infos = super().reset(kwargs)
        self._dt_states = [self._TrajectoryDTState() for _ in range(len(observations["text"]))]
        # Rebuild user prompts using the dt_action / gate templates
        observations["text"] = self.build_text_obs(
            self.pre_text_obs,
    ...
    @property
    def dt_states(self):
        return self._dt_states

    def reset_dt_states(self) -> None:
        self._dt_states = [self._TrajectoryDTState() for _ in range(len(self._dt_states))]

    def env_step_count(self, env_idx: int) -> int:
        return len(self.memory[env_idx])
    ...
    def build_text_obs(
        ...
        # Memory is populated by base.step(); reuse it for the action-history block.
        memory_contexts: List[str] = ["" for _ in range(len(text_obs))]
        valid_lens: List[int] = [0 for _ in range(len(text_obs))]
        if not init and self.config.env.history_length > 0:
            memory_contexts, valid_lens = self.memory.fetch(
                self.config.env.history_length,
                obs_key="text_obs",
        ...
        prompts: List[str] = []
        # `dt_states` may not yet be sized when called from super().reset(); guard.
        if not self._dt_states or len(self._dt_states) != len(text_obs):
            self._dt_states = [self._TrajectoryDTState() for _ in range(len(text_obs))]

        for i in range(len(text_obs)):
            reformatted_admissible = "\n ".join(
                f"'{s}'" for s in admissible_actions[i] if s != "help"
            )
            current_step = len(self.memory[i]) + 1 if not init else 1
            prompts.append(
                build_dt_action_user(
                    init=init or self.config.env.history_length <= 0,
                    task_description=self.tasks[i] if i < len(self.tasks) else "",
                    current_observation=text_obs[i],
                    admissible_actions_str=reformatted_admissible,
                    step_count=len(self.memory[i]),
                    history_length=valid_lens[i],
                    action_history=memory_contexts[i],
                    current_step=current_step,
                    state=self._dt_states[i],
                    suppress_dt_guidance=(self.thinking_mode == "selective_gate"),
                )
            )
        return prompts
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        thinking_mode = getattr(config.env, "thinking_mode", None)
        if thinking_mode in ("dt_action", "selective_gate"):
            manager_cls = AlfWorldSelectiveThinkingEnvironmentManager
        else:
            manager_cls = AlfWorldEnvironmentManager
        envs = manager_cls(_envs, projection_f, config)
        val_envs = manager_cls(_val_envs, projection_f, config)
        return envs, val_envs

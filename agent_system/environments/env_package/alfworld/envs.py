...
# Modified by Anonymous Authors, 2026: added configurable Ray initialisation helpers; rescaled task reward to 1.0.
...
# Keep the temp root short because Ray appends a long session/socket suffix.
_DEFAULT_RAY_TEMP_DIR = "/dev/shm/awray"
_DEFAULT_RAY_PLASMA_DIR = "/dev/shm"
_DEFAULT_RAY_OBJECT_STORE_MEMORY_BYTES = 512 * 1024 * 1024
...
def _init_ray_for_alfworld():
    address = os.environ.get("ALFWORLD_RAY_ADDRESS", "").strip()
    namespace = os.environ.get("ALFWORLD_RAY_NAMESPACE", "").strip() or None
    if address:
        ray.init(
            address=address,
            namespace=namespace,
    ...
    temp_dir = os.environ.get("ALFWORLD_RAY_TEMP_DIR", _DEFAULT_RAY_TEMP_DIR)
    plasma_dir = os.environ.get("ALFWORLD_RAY_PLASMA_DIR", _DEFAULT_RAY_PLASMA_DIR)
    object_store_memory = _get_env_int(
        "ALFWORLD_RAY_OBJECT_STORE_MEMORY_BYTES",
        _DEFAULT_RAY_OBJECT_STORE_MEMORY_BYTES,
    )
    num_cpus = _get_env_int("ALFWORLD_RAY_NUM_CPUS", None)
    num_gpus = _get_env_int("ALFWORLD_RAY_NUM_GPUS", None)
    dashboard_port = _get_env_int("ALFWORLD_RAY_DASHBOARD_PORT", None)
    metrics_export_port = _get_env_int("ALFWORLD_RAY_METRICS_EXPORT_PORT", None)
    include_dashboard = _get_env_bool("ALFWORLD_RAY_INCLUDE_DASHBOARD", False)
    ...
    if metrics_export_port is not None:
        init_kwargs["_metrics_export_port"] = metrics_export_port
...
def compute_reward(info, multi_modal=False):
    if multi_modal:
        reward = 1.0 * float(info['won']) + float(info['goal_condition_success_rate'])
    else:

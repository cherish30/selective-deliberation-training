...
LOG_FILE="${LOG_DIR}/grpo_selective_gate_$(date +%Y%m%d_%H%M%S).log"

# Mode configuration
export THINKING_MODE=selective_gate
export EXPERIMENT_NAME=grpo_qwen3_8b_selective_gate
...
# export RESUME_LORA_PATH=/path/to/checkpoint/actor

echo "Starting Sel.-Gate GRPO training (background)..."
echo "Log: ${LOG_FILE}"
echo "To follow: tail -f ${LOG_FILE}"

nohup bash examples/grpo_trainer/run_alfworld_grpo_selective.sh > "${LOG_FILE}" 2>&1 &
PID=$!
echo "${PID}" > "${LOG_DIR}/grpo_selective_gate_latest.pid"
echo "PID: ${PID}"
